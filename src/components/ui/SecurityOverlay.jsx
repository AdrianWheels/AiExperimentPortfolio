...
export default function SecurityOverlay({
  ...
  // Random delay for desync effect between different puzzle overlays
  ...
  return (
    <div
      className={`absolute inset-0 z-50 overflow-hidden rounded-xl transition-all duration-500 ${className} ${isGlitching ? 'animate-glitch-dissolve pointer-events-none' : 'opacity-100'
        }`}
    >
      ...
      <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-md border border-cyan-500/30 shadow-[0_0_15px_rgba(6,182,212,0.15)]">
        ...
        {/* Scanning Line with glitch effect and desync */}
        <div
          className="absolute inset-x-0 h-[2px] bg-cyan-400/50 shadow-[0_0_10px_#22d3ee] animate-scanline-glitch top-0"
