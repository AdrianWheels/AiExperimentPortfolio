...
const TONE_BANK = [
  { id: 'alpha', label: 'α', description: 'Pulso corto', frequency: 320, color: 'bg-sky-500', borderColor: 'border-sky-300', symbol: '●' },
  { id: 'beta', label: 'β', description: 'Pulso grave', frequency: 180, color: 'bg-indigo-500', borderColor: 'border-indigo-300', symbol: '◆' },
  { id: 'gamma', label: 'γ', description: 'Pulso agudo', frequency: 520, color: 'bg-purple-500', borderColor: 'border-purple-300', symbol: '▲' },
  { id: 'delta', label: 'δ', description: 'Pulso largo', frequency: 260, color: 'bg-emerald-500', borderColor: 'border-emerald-300', symbol: '▬' },
]

// Secuencia lineal que debe completar el usuario (basada en α → δ → γ → β)
...
export default function SoundPatternPuzzle() {
  ...
  return (
    <section
      className={`bg-panel border border-border rounded-lg p-4 flex flex-col gap-3 flex-1 min-h-0 transition-shadow ${
        solved ? 'glow-success' : ''
      }`}
    >
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Presiona cuando cada elemento pase por la zona verde. Orden: α → δ → γ → β
          </p>
        </div>
          ...
          {SEQUENCE_PATTERN.map((element, index) => {
            ...
            return (
              <div
                ...
                className={`absolute top-1/2 w-10 h-10 transform -translate-y-1/2 transition-all duration-100 ${
                  isInTargetZone ? 'scale-125 z-10' : 'scale-100 z-0'
                }`}
                style={{ left: `calc(${elementPosition}% - 20px)` }}
              >
                <div className={`w-full h-full ${tone?.color || 'bg-gray-500'} ${tone?.borderColor || 'border-gray-400'} border-3 rounded-full flex items-center justify-center text-white text-xl font-bold shadow-lg ${
                  isInTargetZone ? 'shadow-white/50' : ''
                }`}>
                  {element.symbol}
                </div>
              </div>
      ...
      <footer className="text-xs text-slate-300/90 bg-black/30 border border-zinc-800 rounded px-3 py-2 font-mono">
        {solved ? (
          ...
          <span>Presiona ACTIVAR cuando cada elemento pase por la zona verde. Secuencia: α → δ → γ → β</span>
        )}
      </footer>
    </section>
  )
