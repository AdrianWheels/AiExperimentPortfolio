...
function ControlsCard() {
  const { setActiveView, triggerBypass, gameState } = useGame()

  return (
    <BentoCard className="h-full">
      <div className="h-full flex flex-col justify-center gap-4 items-center">

        {gameState.portfolioUnlocked ? (
          <button
            onClick={() => setActiveView('portfolio')}
            className="bento-cta text-xs py-2 w-full justify-center max-w-[140px]"
          >
            🎯 Ir al Portfolio
          </button>
        ) : (
          <button
            ...
            className="bento-cta bento-cta-secondary text-xs py-2 w-full justify-center max-w-[140px]"
          >
            🔓 Saltar Puzzles
          </button>
        )}
        ...
        <div className="flex gap-2 justify-center">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="w-9 h-9 rounded-xl bg-card-elevated hover:bg-card-hover border border-white/5 hover:border-white/20 flex items-center justify-center text-gray-400 hover:text-white transition-all group"
          >
